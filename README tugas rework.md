...
Project ini merupakan eksperimen dan evaluasi terhadap dua pendekatan AI yang berbeda untuk menyelesaikan *use case sentiment analysis* terhadap ulasan pelanggan e-commerce. Proyek ini membandingkan (1) model klasik Machine Learning yang dilatih secara lokal menggunakan Scikit-learn, dan (2) Large Language Model (LLM) yang diakses melalui Gemini AI API menggunakan pendekatan *prompting*. Kedua pendekatan dijalankan pada data uji (*test set*) yang sama untuk mengukur kelebihan, keterbatasan, dan *trade-off* guna menyusun rekomendasi teknis berbasis data.
...
- **Target Label:** `sentiment` (positif / negatif)
...
## 🔍 Kondisi Awal Dataset & Permasalahan yang Ditemukan
...
| **Karakteristik Teks** | Memuat variasi bahasa Indonesia kasual, ulasan pendek, hingga ulasan dengan ambiguitas kata kunci |
| **Metode Evaluasi** | Dipisahkan menggunakan *Stratified Split* (80% Train, 20% Test) dengan `random_state=42` |
| **Ukuran Data Uji (*Test Data*)**| **40 baris data** (Digunakan secara konsisten untuk mengevaluasi kedua model) |

---

## 🧹 Eksperimen Pendekatan 1 - Model Klasik (Scikit-learn)

Berikut adalah rincian tahapan pemrosesan dan ekstraksi fitur yang diterapkan pada model klasik:

| Komponen | Metode yang Digunakan | Alasan Pemilihan Metode |
| :--- | :--- | :--- |
| **Text Preprocessing** | `TfidfVectorizer(max_features=1000)` | Mengubah teks ulasan menjadi representasi numerik berdasarkan bobot frekuensi kata (TF-IDF) untuk menangkap kata kunci dominan. |
| **Algoritma Model** | `Logistic Regression` | Algoritma klasifikasi linier yang sangat efisien, cepat, dan bekerja baik pada teks dengan representasi berbasis TF-IDF. |
...
Proses klasifikasi teks berbasis *Generative AI* diimplementasikan menggunakan pemanggilan Gemini API:
...
*   **LLM API:** Sangat bergantung pada kestabilan *prompt* dan rentan mengalami *error* jaringan internet. Jika struktur atau format masukan berubah sedikit, ada kemungkinan format respons tidak seragam.
...
Berdasarkan data eksperimen, kami merekomendasikan **Pendekatan Hibrida (Hybrid Approach) atau Model Klasik yang Dioptimalkan** untuk sistem produksi aplikasi e-commerce sesungguhnya dengan pertimbangan komersial:
...
2. Clone repository ini ke direktori lokal Anda:
   ```bash
   git clone https://github.com/username/assignment-model-experiment-nama-peserta.git
